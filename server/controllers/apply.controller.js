  ...
  try {
    ...
    if (training_id) {
      ...
      if (isAlreadyApplied) {
        return res
          .status(400)
          .json({ error: "Student already applied for this training" });
      }
...
const withdrawApply = async (req, res) => {
  ...
  try {
    if (job_id && uid) {
      ...
      jobPost.candidates.splice(
        jobPost.candidates.map((candidate) => candidate.uid).indexOf(uid),
        1
      );
      student.applications.splice(
    ...
    if (training_id && uid) {
      ...
      training.attendees.splice(
        training.attendees.map((attendee) => attendee.uid).indexOf(uid),
        1
      );
      student.trainingApplications.splice(
        student.trainingApplications
          .map((application) => application.training_id)
          .indexOf(training_id),
        1
      );
      await training.save();
...
const getAppliedStudents = async (req, res) => {
  try {
    ...
    if (result.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: `No ${applicationType} post found` });
    }
